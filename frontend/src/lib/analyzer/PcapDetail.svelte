<script lang="ts">
    ...
    import { GetPacketDetail, GetPacketsByPage, GetPacketHex } from '../../../wailsjs/go/main/App';
    ...
    let packetTotal = 0;
    $: packetTotalPages = Math.ceil(packetTotal / packetPageSize) || 1;
    ...
    async function loadAnalyzeData() {
        ...
        packets = [];
        try {
            const resStr = await GetPacketsByPage(file.filePath, packetPage, packetPageSize);
            const resData = JSON.parse(resStr);
            if (resData && resData.list) {
                packetTotal = resData.total || 0;
    ...
    <div class="level-box level-2">
        <div class="level-header flex-between">
            <h3><span class="level-num">2</span> 文件帧提取 (Frames)
                <span class="sub-title">共计 {packetTotal} 帧</span>
            </h3>

            <div class="inline-filter">
                <input type="text" bind:value={filterProtocol} placeholder="协议 (如 TCP)"/>
                <input type="text" bind:value={filterIp} placeholder="IP (源或目的)"/>
                <input type="text" bind:value={filterInfo} placeholder="Info 关键字"/>
                <button class="action-btn outline" on:click={resetFilters}>清空</button>
            </div>
        </div>
        ...
        <div class="pagination">
            <div class="page-info">
                <span>分页拉取: (第 {packetPage} / {packetTotalPages} 页)</span>
            ...
            <div class="page-controls">
                ...
                <span class="jump-box">
                    跳至 <input type="number" min="1" max={packetTotalPages} bind:value={jumpPageNum} on:keyup={(e) => e.key === 'Enter' && changePacketPage(jumpPageNum)} /> 页
                </span>

                <button disabled={packetPage >= packetTotalPages} on:click={() => changePacketPage(packetPage + 1)}>下一页</button>
                <button disabled={packetPage >= packetTotalPages} on:click={() => changePacketPage(packetTotalPages)}>尾页</button>
            </div>
        </div>
    </div>
...
<style>
    .detail-container {
        ...
        height: 100%;
        gap: 20px;           /* 增大三个模块间的间距，呼吸感更强 */
        padding-bottom: 24px;
        overflow-y: auto;    /* 开启纵向滚动 */
        padding-right: 8px;  /* 给右侧滚动条留出间隙 */
    }
    ...
    .level-box {
        ...
        flex-shrink: 0; /* 【核心修改 2】：禁止盒子被父容器强行压缩挤扁 */
    }
    ...
    /* ======== 层级 2: 文件帧流 ======== */
    .level-2 {
        min-height: 360px; /* 给定充足的最小高度，保证表格至少能看 10 多行 */
        flex: 1 0 auto;    /* 如果屏幕巨大（如4K），允许它继续延伸 */
    }
    .inline-filter { display: flex; gap: 8px; }
    .inline-filter input {
        background: #1e293b; border: 1px solid #334155; color: white;
        padding: 4px 8px; border-radius: 4px; outline: none; font-size: 0.8rem; width: 110px; transition: 0.2s;
    }
    .inline-filter input:focus { border-color: #4f46e5; }
    .action-btn.outline { background: transparent; border: 1px solid #475569; color: #cbd5e1; padding: 4px 10px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; }
    ...
    .pagination {
        ...
        padding: 6px 16px; background: #0f172a; border-top: 1px solid #1e293b;
    ...
    .size-selector {
        ...
        padding: 2px 6px; border-radius: 4px; outline: none; font-size: 0.75rem;
        cursor: pointer;
    }
    ...
    .jump-box input {
        background: #1e293b; border: 1px solid #334155; color: white;
        width: 40px; padding: 3px; border-radius: 4px; text-align: center;
        outline: none; font-size: 0.8rem;
    }
    .jump-box input:focus { border-color: #4f46e5; }
    /* 隐藏数字输入框自带的上下小箭头 */
    .jump-box input::-webkit-outer-spin-button,
    .jump-box input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }

    /* ======== 层级 3: 帧系列透视 ======== */
    .level-3 {
        min-height: 550px; /* 因为要展示长长的 Hex 流，给它最高的基础高度 */
        flex: 1 0 auto;
    }
